// Test_Quad.cpp
// Test driver for Quad ADT.
// Modified by Anne Liao (40215517) on July 10, 2022.
// Added void test_exceptions();
...
void test_exceptions()
{
	...
	// accessing out of bounds index error
	exceptionThrown = false;
	try { double x{ q[0] }; }
	catch (std::out_of_range) { exceptionThrown = true; }
	assert(exceptionThrown);
	...
	// accessing out of bounds function objects error
	exceptionThrown = false;
	try { double x{ q(0) }; }
	catch (std::out_of_range) { exceptionThrown = true; }
	assert(exceptionThrown);
	...
	// accessing out of bounds function objects error
	exceptionThrown = false;
	try { double x{ q(1,2,5) }; }
	catch (std::out_of_range) { exceptionThrown = true; }
	assert(exceptionThrown);

	// accessing out of bounds function objects error
	exceptionThrown = false;
	try { double x{ q(1,2,0) }; }
	catch (std::out_of_range) { exceptionThrown = true; }
	assert(exceptionThrown);

	// accessing out of bounds function objects error
	exceptionThrown = false;
	try { double x{ q(1,2,3,5) }; }
	catch (std::out_of_range) { exceptionThrown = true; }
	assert(exceptionThrown);

	// accessing out of bounds function objects error
	exceptionThrown = false;
	try { double x{ q(1,2,3,0) }; }
	catch (std::out_of_range) { exceptionThrown = true; }
	assert(exceptionThrown);
}